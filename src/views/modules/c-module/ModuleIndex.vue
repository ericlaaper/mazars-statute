...
<script>
import sectionData from "@/assets/modules/sectionC.json";
import stats from "@/assets/modules/moduleC.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/c-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/c-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/c-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/c-module/StepComponentC.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (this.moduleData.step === 0) {
        await this.$store.commit("moduleC/stepUpNr", 1);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        !this.visData.section_3
      ) {
        await this.$store.commit("moduleC/update_ready_a", true);
        await this.$store.commit("moduleC/stepUpNr", 3);
      } else if (this.moduleData.step === 1 && !this.visData.section_2) {
        await this.$store.commit("moduleC/update_ready_a", true);
        await this.$store.commit("moduleC/stepUpNr", 2);
      } else if (this.moduleData.step === 1) {
        await this.$store.commit("moduleC/update_ready_a", true);
        await this.$store.commit("moduleC/stepUpNr", 1);
      } else if (this.moduleData.step === 2 && !this.visData.section_3) {
        await this.$store.commit("moduleC/update_ready_b", true);
        await this.$store.commit("moduleC/stepUpNr", 2);
      } else if (this.moduleData.step === 2) {
        await this.$store.commit("moduleC/update_ready_b", true);
        await this.$store.commit("moduleC/stepUpNr", 1);
      } else if (this.moduleData.step === 3) {
        await this.$store.commit("moduleC/update_ready_c", true);
        await this.$store.commit("moduleC/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleC/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
        ...
        await this.$store.commit("moduleC/stepDownNr", 3);
      } else if (this.moduleData.step === 4 && !this.visData.section_3) {
        await this.$store.commit("moduleC/stepDownNr", 2);
      } else if (this.moduleData.step) {
        await this.$store.commit("moduleC/stepDownNr", 1);
    ...
    backStart() {
      this.$store.commit("moduleC/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleC/update_finished", true);
      await this.$store.dispatch("moduleC/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportC(payload)
        .then(() => {})
        .finally(() => {
  ...
  async created() {
    ...
    await this.$store.dispatch("visC/getVisData", payload);
    await this.$store.dispatch("moduleC/getModulesData", payload);
  },
};
</script>

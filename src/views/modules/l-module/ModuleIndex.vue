...
<script>
import sectionData from "@/assets/modules/sectionL.json";
import stats from "@/assets/modules/moduleL.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/l-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/l-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/l-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/l-module/StepComponentC.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (this.moduleData.step === 0) {
        await this.$store.commit("moduleL/stepUpNr", 1);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        !this.visData.section_3
      ) {
        await this.$store.commit("moduleL/update_ready_a", true);
        await this.$store.commit("moduleL/stepUpNr", 3);
      } else if (this.moduleData.step === 1 && !this.visData.section_2) {
        await this.$store.commit("moduleL/update_ready_a", true);
        await this.$store.commit("moduleL/stepUpNr", 2);
      } else if (this.moduleData.step === 1) {
        await this.$store.commit("moduleL/update_ready_a", true);
        await this.$store.commit("moduleL/stepUpNr", 1);
      } else if (this.moduleData.step === 2 && !this.visData.section_3) {
        await this.$store.commit("moduleL/update_ready_b", true);
        await this.$store.commit("moduleL/stepUpNr", 2);
      } else if (this.moduleData.step === 2) {
        await this.$store.commit("moduleL/update_ready_b", true);
        await this.$store.commit("moduleL/stepUpNr", 1);
      } else if (this.moduleData.step === 3) {
        await this.$store.commit("moduleL/update_ready_c", true);
        await this.$store.commit("moduleL/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleL/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
        ...
        await this.$store.commit("moduleL/stepDownNr", 3);
      } else if (this.moduleData.step === 4 && !this.visData.section_3) {
        await this.$store.commit("moduleL/stepDownNr", 2);
      } else if (this.moduleData.step) {
        await this.$store.commit("moduleL/stepDownNr", 1);
    ...
    backStart() {
      this.$store.commit("moduleL/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleL/update_finished", true);
      await this.$store.dispatch("moduleL/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportL(payload)
        .then(() => {})
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters("visL", ["visData"]),
    ...mapGetters("moduleL", ["moduleData"]),
  },
  async created() {
    const payload = {
      email: this.email,
    ...
    await this.$store.dispatch("visL/getVisData", payload);
    await this.$store.dispatch("moduleL/getModulesData", payload);
  },
};
</script>

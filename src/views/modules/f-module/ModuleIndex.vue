            ...
            >
              <v-form @submit.prevent="onSubmit">
                ...
                <StepComponentE
                  ...
                  :statText="stats"
                />
                <div class="pt-15" v-else-if="moduleData.step === 6">
...
<script>
import sectionData from "@/assets/modules/sectionF.json";
import stats from "@/assets/modules/moduleF.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/f-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/f-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/f-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/f-module/StepComponentC.vue";
import StepComponentD from "@/views/modules/f-module/StepComponentD.vue";
import StepComponentE from "@/views/modules/f-module/StepComponentE.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        ...
        await this.$store.commit("moduleF/stepUpNr", 6);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleF/stepUpNr", 5);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleF/stepUpNr", 4);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleF/stepUpNr", 3);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        this.visData.section_2
      ) {
        await this.$store.commit("moduleF/stepUpNr", 2);
      } else if (this.moduleData.step === 0 && this.visData.section_1) {
        await this.$store.commit("moduleF/stepUpNr", 1);
      } else if (
        ...
        await this.$store.commit("moduleF/update_ready_a", true);
        await this.$store.commit("moduleF/stepUpNr", 5);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleF/update_ready_b", true);
        await this.$store.commit("moduleF/stepUpNr", 4);
      } else if (
        this.moduleData.step === 2 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleF/update_ready_c", true);
        await this.$store.commit("moduleF/stepUpNr", 3);
      } else if (
        this.moduleData.step === 3 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleF/update_ready_d", true);
        await this.$store.commit("moduleF/stepUpNr", 2);
      } else if (this.moduleData.step === 4 && this.visData.section_5) {
        await this.$store.commit("moduleF/update_ready_d", true);
        await this.$store.commit("moduleF/stepUpNr", 1);
      } else if (this.moduleData.step === 5) {
        await this.$store.commit("moduleF/update_ready_e", true);
        await this.$store.commit("moduleF/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleF/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
        ...
        await this.$store.commit("moduleF/stepDownNr", 6);
      } else if (
        this.moduleData.step === 6 &&
        !this.visData.section_5 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleF/stepDownNr", 5);
      } else if (
        this.moduleData.step === 6 &&
        !this.visData.section_5 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleF/stepDownNr", 4);
      } else if (
        this.moduleData.step === 6 &&
        !this.visData.section_5 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleF/stepDownNr", 3);
      } else if (
        this.moduleData.step === 6 &&
        !this.visData.section_5 &&
        this.visData.section_4
      ) {
        await this.$store.commit("moduleF/stepDownNr", 2);
      } else if (this.moduleData.step === 6 && this.visData.section_5) {
        await this.$store.commit("moduleF/stepDownNr", 1);
      } else if (
    ...
    backStart() {
      this.$store.commit("moduleF/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleF/update_finished", true);
      await this.$store.dispatch("moduleF/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportF(payload)
        .then(() => {})
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters("visF", ["visData"]),
    ...mapGetters("moduleF", ["moduleData"]),
  },
  async created() {
    const payload = {
      email: this.email,
    ...
    await this.$store.dispatch("visF/getVisData", payload);
    await this.$store.dispatch("moduleF/getModulesData", payload);
  },
};
</script>

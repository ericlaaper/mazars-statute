...
<script>
import sectionData from "@/assets/modules/sectionI.json";
import stats from "@/assets/modules/moduleI.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/i-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/i-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/i-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/i-module/StepComponentC.vue";
import StepComponentD from "@/views/modules/i-module/StepComponentD.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        ...
        await this.$store.commit("moduleI/stepUpNr", 5);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleI/stepUpNr", 4);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleI/stepUpNr", 3);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        this.visData.section_2
      ) {
        await this.$store.commit("moduleI/stepUpNr", 2);
      } else if (this.moduleData.step === 0 && this.visData.section_1) {
        await this.$store.commit("moduleI/stepUpNr", 1);
      } else if (
        ...
        await this.$store.commit("moduleI/update_ready_a", true);
        await this.$store.commit("moduleI/stepUpNr", 4);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleI/update_ready_b", true);
        await this.$store.commit("moduleI/stepUpNr", 3);
      } else if (
        this.moduleData.step === 2 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleI/update_ready_c", true);
        await this.$store.commit("moduleI/stepUpNr", 2);
      } else if (this.moduleData.step === 3 && this.visData.section_4) {
        await this.$store.commit("moduleI/update_ready_c", true);
        await this.$store.commit("moduleI/stepUpNr", 1);
      } else if (this.moduleData.step === 4) {
        await this.$store.commit("moduleI/update_ready_d", true);
        await this.$store.commit("moduleI/stepUpNr", 1);
      }
    ...
    async stepBack() {
      ...
      if (
        ...
        await this.$store.commit("moduleI/stepDownNr", 5);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_4 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleI/stepDownNr", 4);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_4 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleI/stepDownNr", 3);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_4 &&
        this.visData.section_3
      ) {
        await this.$store.commit("moduleI/stepDownNr", 2);
      } else if (this.moduleData.step === 5 && this.visData.section_4) {
        await this.$store.commit("moduleI/stepDownNr", 1);
      } else if (
    ...
    getReport() {
      ...
      ReportService.getReportI(payload)
        .then(() => {})
        .finally(() => {
  ...
  async created() {
    ...
    await this.$store.dispatch("visI/getVisData", payload);
    await this.$store.dispatch("moduleI/getModulesData", payload);
  },
};
</script>

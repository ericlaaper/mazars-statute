...
<script>
import sectionData from "@/assets/modules/sectionB.json";
import stats from "@/assets/modules/moduleB.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/b-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/b-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/b-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/b-module/StepComponentC.vue";
import StepComponentD from "@/views/modules/b-module/StepComponentD.vue";
import StepComponentE from "@/views/modules/b-module/StepComponentE.vue";
import StepComponentF from "@/views/modules/b-module/StepComponentF.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        ...
        await this.$store.commit("moduleB/stepUpNr", 7);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleB/stepUpNr", 6);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleB/stepUpNr", 5);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleB/stepUpNr", 4);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleB/stepUpNr", 3);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        this.visData.section_2
      ) {
        await this.$store.commit("moduleB/stepUpNr", 2);
      } else if (this.moduleData.step === 0 && this.visData.section_1) {
        await this.$store.commit("moduleB/stepUpNr", 1);
      } else if (
        ...
        await this.$store.commit("moduleB/update_ready_a", true);
        await this.$store.commit("moduleB/stepUpNr", 6);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleB/update_ready_b", true);
        await this.$store.commit("moduleB/stepUpNr", 5);
      } else if (
        this.moduleData.step === 2 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleB/update_ready_c", true);
        await this.$store.commit("moduleB/stepUpNr", 4);
      } else if (
        this.moduleData.step === 3 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleB/update_ready_d", true);
        await this.$store.commit("moduleB/stepUpNr", 3);
      } else if (
        this.moduleData.step === 4 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleB/update_ready_e", true);
        await this.$store.commit("moduleB/stepUpNr", 2);
      } else if (this.moduleData.step === 5 && this.visData.section_6) {
        await this.$store.commit("moduleB/update_ready_e", true);
        await this.$store.commit("moduleB/stepUpNr", 1);
      } else if (this.moduleData.step === 6) {
        await this.$store.commit("moduleB/update_ready_f", true);
        await this.$store.commit("moduleB/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleB/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
    ...
    backStart() {
      this.$store.commit("moduleB/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleB/update_finished", true);
      await this.$store.dispatch("moduleB/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportB(payload)
        .then(() => {})
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters("visB", ["visData"]),
    ...mapGetters("moduleB", ["moduleData"]),
  },
  async created() {
    const payload = {
      email: this.email,
    ...
    await this.$store.dispatch("visB/getVisData", payload);
    await this.$store.dispatch("moduleB/getModulesData", payload);
  },
};
</script>

...
<script>
import sectionData from "@/assets/modules/sectionD.json";
import stats from "@/assets/modules/moduleD.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/d-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/d-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/d-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/d-module/StepComponentC.vue";
import StepComponentD from "@/views/modules/d-module/StepComponentD.vue";
import StepComponentE from "@/views/modules/d-module/StepComponentE.vue";
import StepComponentF from "@/views/modules/d-module/StepComponentF.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        ...
        await this.$store.commit("moduleD/stepUpNr", 7);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleD/stepUpNr", 6);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleD/stepUpNr", 5);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleD/stepUpNr", 4);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleD/stepUpNr", 3);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        this.visData.section_2
      ) {
        await this.$store.commit("moduleD/stepUpNr", 2);
      } else if (this.moduleData.step === 0 && this.visData.section_1) {
        await this.$store.commit("moduleD/stepUpNr", 1);
      } else if (
        ...
        await this.$store.commit("moduleD/update_ready_a", true);
        await this.$store.commit("moduleD/stepUpNr", 6);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleD/update_ready_b", true);
        await this.$store.commit("moduleD/stepUpNr", 5);
      } else if (
        this.moduleData.step === 2 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleD/update_ready_c", true);
        await this.$store.commit("moduleD/stepUpNr", 4);
      } else if (
        this.moduleData.step === 3 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleD/update_ready_d", true);
        await this.$store.commit("moduleD/stepUpNr", 3);
      } else if (
        this.moduleData.step === 4 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleD/update_ready_e", true);
        await this.$store.commit("moduleD/stepUpNr", 2);
      } else if (this.moduleData.step === 5 && this.visData.section_6) {
        await this.$store.commit("moduleD/update_ready_e", true);
        await this.$store.commit("moduleD/stepUpNr", 1);
      } else if (this.moduleData.step === 6) {
        await this.$store.commit("moduleD/update_ready_f", true);
        await this.$store.commit("moduleD/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleD/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
        ...
        await this.$store.commit("moduleD/stepDownNr", 7);
      } else if (
        this.moduleData.step === 7 &&
        !this.visData.section_6 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleD/stepDownNr", 6);
      } else if (
        this.moduleData.step === 7 &&
        !this.visData.section_6 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleD/stepDownNr", 5);
      } else if (
        this.moduleData.step === 7 &&
        !this.visData.section_6 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleD/stepDownNr", 4);
      } else if (
        this.moduleData.step === 7 &&
        !this.visData.section_6 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleD/stepDownNr", 3);
      } else if (
        this.moduleData.step === 7 &&
        !this.visData.section_6 &&
        this.visData.section_5
      ) {
        await this.$store.commit("moduleD/stepDownNr", 2);
      } else if (this.moduleData.step === 7 && this.visData.section_6) {
        await this.$store.commit("moduleD/stepDownNr", 1);
      } else if (
    ...
    backStart() {
      this.$store.commit("moduleD/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleD/update_finished", true);
      await this.$store.dispatch("moduleD/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportD(payload)
        .then(() => {})
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters("visD", ["visData"]),
    ...mapGetters("moduleD", ["moduleData"]),
  },
  async created() {
    const payload = {
      email: this.email,
    ...
    await this.$store.dispatch("visD/getVisData", payload);
    await this.$store.dispatch("moduleD/getModulesData", payload);
  },
};
</script>

...
<script>
import sectionData from "@/assets/modules/sectionK.json";
import stats from "@/assets/modules/moduleK.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/k-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/k-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/k-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/k-module/StepComponentC.vue";
import StepComponentD from "@/views/modules/k-module/StepComponentD.vue";
import StepComponentE from "@/views/modules/k-module/StepComponentE.vue";
import StepComponentF from "@/views/modules/k-module/StepComponentF.vue";
import StepComponentG from "@/views/modules/k-module/StepComponentG.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        ...
        await this.$store.commit("moduleK/stepUpNr", 8);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleK/stepUpNr", 7);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleK/stepUpNr", 6);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleK/stepUpNr", 5);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleK/stepUpNr", 4);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleK/stepUpNr", 3);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        this.visData.section_2
      ) {
        await this.$store.commit("moduleK/stepUpNr", 2);
      } else if (this.moduleData.step === 0 && this.visData.section_1) {
        await this.$store.commit("moduleK/stepUpNr", 1);
      } else if (
        ...
        await this.$store.commit("moduleK/update_ready_a", true);
        await this.$store.commit("moduleK/stepUpNr", 7);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleK/update_ready_b", true);
        await this.$store.commit("moduleK/stepUpNr", 6);
      } else if (
        this.moduleData.step === 2 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleK/update_ready_c", true);
        await this.$store.commit("moduleK/stepUpNr", 5);
      } else if (
        this.moduleData.step === 3 &&
        !this.visData.section_4 &&
        ...
        await this.$store.commit("moduleK/update_ready_d", true);
        await this.$store.commit("moduleK/stepUpNr", 4);
      } else if (
        this.moduleData.step === 4 &&
        !this.visData.section_5 &&
        ...
        await this.$store.commit("moduleK/update_ready_e", true);
        await this.$store.commit("moduleK/stepUpNr", 3);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_6 &&
        ...
        await this.$store.commit("moduleK/update_ready_f", true);
        await this.$store.commit("moduleK/stepUpNr", 2);
      } else if (this.moduleData.step === 6 && this.visData.section_7) {
        await this.$store.commit("moduleK/update_ready_f", true);
        await this.$store.commit("moduleK/stepUpNr", 1);
      } else if (this.moduleData.step === 7) {
        await this.$store.commit("moduleK/update_ready_g", true);
        await this.$store.commit("moduleK/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleK/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
        ...
        await this.$store.commit("moduleK/stepDownNr", 8);
      } else if (
        this.moduleData.step === 8 &&
        !this.visData.section_7 &&
        !this.visData.section_6 &&
        ...
        await this.$store.commit("moduleK/stepDownNr", 7);
      } else if (
        this.moduleData.step === 8 &&
        !this.visData.section_7 &&
        !this.visData.section_6 &&
        ...
        await this.$store.commit("moduleK/stepDownNr", 6);
      } else if (
        this.moduleData.step === 8 &&
        !this.visData.section_7 &&
        !this.visData.section_6 &&
        ...
        await this.$store.commit("moduleK/stepDownNr", 5);
      } else if (
        this.moduleData.step === 8 &&
        !this.visData.section_7 &&
        !this.visData.section_6 &&
        ...
        await this.$store.commit("moduleK/stepDownNr", 4);
      } else if (
        this.moduleData.step === 8 &&
        !this.visData.section_7 &&
        !this.visData.section_6 &&
        ...
        await this.$store.commit("moduleK/stepDownNr", 3);
      } else if (
        this.moduleData.step === 8 &&
        !this.visData.section_7 &&
        this.visData.section_6
      ) {
        await this.$store.commit("moduleK/stepDownNr", 2);
      } else if (this.moduleData.step === 8 && this.visData.section_7) {
        await this.$store.commit("moduleK/stepDownNr", 1);
      } else if (
    ...
    backStart() {
      this.$store.commit("moduleK/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleK/update_finished", true);
      await this.$store.dispatch("moduleK/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportK(payload)
        .then(() => {})
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters("visK", ["visData"]),
    ...mapGetters("moduleK", ["moduleData"]),
  },
  async created() {
    const payload = {
      email: this.email,
    ...
    await this.$store.dispatch("visK/getVisData", payload);
    await this.$store.dispatch("moduleK/getModulesData", payload);
  },
};
</script>

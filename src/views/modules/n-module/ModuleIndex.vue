            ...
            >
              <v-form @submit.prevent="onSubmit">
                ...
                <StepComponentD
                  ...
                  :statText="stats"
                />
                <div class="pt-15" v-else-if="moduleData.step === 5">
...
<script>
import sectionData from "@/assets/modules/sectionN.json";
import stats from "@/assets/modules/moduleN.json";
import vimeo from "@/assets/vimeo/vimeo.json";
import StepComponentStart from "@/views/modules/n-module/StepComponentStart.vue";
import StepComponentA from "@/views/modules/n-module/StepComponentA.vue";
import StepComponentB from "@/views/modules/n-module/StepComponentB.vue";
import StepComponentC from "@/views/modules/n-module/StepComponentC.vue";
import StepComponentD from "@/views/modules/n-module/StepComponentD.vue";
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        ...
        await this.$store.commit("moduleN/stepUpNr", 5);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleN/stepUpNr", 4);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleN/stepUpNr", 3);
      } else if (
        this.moduleData.step === 0 &&
        !this.visData.section_1 &&
        this.visData.section_2
      ) {
        await this.$store.commit("moduleN/stepUpNr", 2);
      } else if (this.moduleData.step === 0 && this.visData.section_1) {
        await this.$store.commit("moduleN/stepUpNr", 1);
      } else if (
        ...
        await this.$store.commit("moduleN/update_ready_a", true);
        await this.$store.commit("moduleN/stepUpNr", 4);
      } else if (
        this.moduleData.step === 1 &&
        !this.visData.section_2 &&
        ...
        await this.$store.commit("moduleN/update_ready_b", true);
        await this.$store.commit("moduleN/stepUpNr", 3);
      } else if (
        this.moduleData.step === 2 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleN/update_ready_c", true);
        await this.$store.commit("moduleN/stepUpNr", 2);
      } else if (this.moduleData.step === 3 && this.visData.section_4) {
        await this.$store.commit("moduleN/update_ready_c", true);
        await this.$store.commit("moduleN/stepUpNr", 1);
      } else if (this.moduleData.step === 4) {
        await this.$store.commit("moduleN/update_ready_d", true);
        await this.$store.commit("moduleN/stepUpNr", 1);
      }
      await this.$store.dispatch("moduleN/setModulesData");
    },
    async stepBack() {
      await this.$vuetify.goTo(0);
      if (
        ...
        await this.$store.commit("moduleN/stepDownNr", 5);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_4 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleN/stepDownNr", 4);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_4 &&
        !this.visData.section_3 &&
        ...
        await this.$store.commit("moduleN/stepDownNr", 3);
      } else if (
        this.moduleData.step === 5 &&
        !this.visData.section_4 &&
        this.visData.section_3
      ) {
        await this.$store.commit("moduleN/stepDownNr", 2);
      } else if (this.moduleData.step === 5 && this.visData.section_4) {
        await this.$store.commit("moduleN/stepDownNr", 1);
      } else if (
    ...
    backStart() {
      this.$store.commit("moduleN/update_step", 0);
    },
    async getReport() {
      await this.$store.commit("moduleN/update_finished", true);
      await this.$store.dispatch("moduleN/setModulesData");
      const payload = {
        email: this.email,
      };
      ...
      ReportService.getReportN(payload)
        .then(() => {})
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters("visN", ["visData"]),
    ...mapGetters("moduleN", ["moduleData"]),
  },
  async created() {
    const payload = {
      email: this.email,
    ...
    await this.$store.dispatch("visN/getVisData", payload);
    await this.$store.dispatch("moduleN/getModulesData", payload);
  },
};
</script>
